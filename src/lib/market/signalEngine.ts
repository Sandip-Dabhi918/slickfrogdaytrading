...
const isSellFade =
  smoothedVelocity > 0 &&
  smoothedAcceleration < -0.1; // add threshold

const isBuyExhaustion =
  smoothedVelocity < 0 &&
  smoothedAcceleration > 0.1;

  if (Math.abs(smoothedAcceleration) > 0.3) {
  score += 1;
}

if (spread == null) {
  if (Math.abs(smoothedVelocity) > 0.5) {
    score += 1;
  }
}
